...
const Tiptap: React.FC = () => {
  ...
  const [isGeneratingTextFromContext, setIsGeneratingTextFromContext] =
    useState(false); // Für den Ladezustand
  ...
  const handleContextMenuGenerateText = async (
    ...
    const effectivePrompt =
      ...
      "Schreibe einen passenden wissenschaftlichen Textabschnitt...";
    try {
      ...
      if (response.generated_text) {
        ...
        while ((match = citationRegex.exec(response.generated_text)) !== null) {
          ...
          if (sourceData && editor.schema.nodes.citation) {
            const attrs = {
              ...
              displayText: `(${sourceData.author || "A"}, ${
                sourceData.year || "Y"
              })`,
            };
            const node = editor.schema.nodes.citation.create(attrs);
            if (node) contentToInsert.push(node.toJSON());
        ...
        if (contentToInsert.length > 0) {
          ...
          for (const item of contentToInsert) {
            chain = chain.insertContent(item);
          }
          chain.run();
        }
      } else {
  ...
  const handleContextMenuRewriteText = (prompt?: string) => {
    ...
    alert(
      `Aktion 'Umschreiben' aus Kontextmenü getriggert. Prompt: ${
        prompt || "(kein Prompt)"
      }. Markierter Text: ${editor.state.selection.empty ? "Nichts" : "Ja"}`
    );
  ...
  const handleContextMenuFindSources = () => {
    ...
    alert(
      "Aktion 'Quellen suchen' aus Kontextmenü getriggert. (Logik zum Öffnen des Popovers fehlt noch)"
    );
  };
