...
const EditorBubbleMenu: React.FC<EditorBubbleMenuProps> = ({ editor }) => {
  ...
  const [selectedRangeForCitation, setSelectedRangeForCitation] = useState<{
    from: number;
    to: number;
  } | null>(null);

  // Für Inline Prompts
  const [activePrompt, setActivePrompt] = useState<ActivePromptType>(
    ActivePromptType.NONE
  );
  const [currentInlinePrompt, setCurrentInlinePrompt] = useState("");
  const [rangeForInsertion, setRangeForInsertion] = useState<{
    from: number;
    to: number;
  } | null>(null);

  const toggleInlinePrompt = (type: ActivePromptType) => {
    // Stelle sicher, dass diese Funktion existiert
    if (!editor) return;
    ...
    if (activePrompt === type) {
      ...
      if (type === ActivePromptType.REWRITE) {
        ...
        const selectedText = editor.state.doc
          ...
          .trim();
        setCurrentInlinePrompt(
          `Umschreibung für: "${selectedText.substring(
            0,
            30
          )}..." (optional verbessern)`
        );
  ...
  const handleRewriteText = async (optionalPrompt?: string) => {
    if (!editor || editor.state.selection.empty) {
      if (activePrompt === ActivePromptType.REWRITE)
        setActivePrompt(ActivePromptType.NONE); // Schließe Prompt-Feld, wenn keine Selektion
    ...
    try {
      ...
      if (rewrittenText && !rewrittenText.toLowerCase().startsWith("fehler:")) {
        editor
          ...
          .deleteSelection()
          .insertContent(rewrittenText.trim())
          .run();
      } else {
  ...
  const handleSelectSource = (source: SourceChunk) => {
    // 'source' ist vom Typ SourceChunk
    if (!editor) return;
    ...
    const citationNodeAttributes = {
      ...
      displayText: `(${source.document_author || "Unbekannt"}, ${
        source.publication_year || "N.D."
      }${
        source.page_number !== null ? `, S. ${source.page_number + 1}` : ""
      }) `,
    };
    console.log(
      "[handleSelectSource] Erstellte citationNodeAttributes:",
  ...
  const handleGenerateText = async (userPrompt?: string) => {
    ...
    const effectiveQueryForAPI =
      userPrompt?.trim() ||
      "Schreibe einen passenden wissenschaftlichen Textabschnitt, der den vorherigen Inhalt fortführt oder ergänzt, basierend auf den relevanten Quellen.";
    ...
    try {
      ...
      if (response.generated_text) {
        ...
        while ((match = citationRegex.exec(response.generated_text)) !== null) {
          matchCount++;
          const plainTextBefore = response.generated_text.substring(
            lastIndex,
            match.index
          );
          if (plainTextBefore) {
          ...
          if (sourceData) {
            const citationAttributes = {
              ...
              displayText: `(${sourceData.author || "Autor unbekannt"}, ${
                sourceData.year || "N.D."
              }${
                sourceData.page !== null ? `, S. ${sourceData.page! + 1}` : ""
              })`,
            };

            if (editor.schema.nodes.citation) {
              const citationNodeInstance =
                editor.schema.nodes.citation.create(citationAttributes);
        ...
        if (contentToInsert.length > 0) {
          ...
          const insertPos = rangeForInsertion
            ? rangeForInsertion.to
    ...
    <>
      ...
      >
        <div className="flex space-x-0.5">
          {" "}
          {/* Reihe für die Haupt-Aktionsbuttons */}
          {/* Fett */}
          <button
          ...
          <button
            ...
            disabled={
              !editor || isGeneratingText || editor.state.selection.empty
            }
            ...
            title="Text umschreiben (Schnell)"
            type="button"
          >
          ...
          <button
            ...
            className={`p-1 h-9 w-5 flex items-center justify-center rounded ${
              activePrompt === ActivePromptType.REWRITE
          ...
          <button
            ...
            disabled={
              !editor || isSourceLoading || editor.state.selection.empty
            }
          ...
          <button
            ...
            disabled={!editor}
            className={`p-1 h-9 w-5 flex items-center justify-center rounded ${
              activePrompt === ActivePromptType.GENERATE
        ...
        <div className="inline-prompt-container w-full">
          {" "}
          {/* Stellt sicher, dass der Container die Breite nimmt */}
          <InlinePromptInput
